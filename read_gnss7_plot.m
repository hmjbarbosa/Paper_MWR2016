...
for remST=1:19
  ...
  for bstrap=1:100
    ...
    for t=mintime:maxtime
	  ...
	  nlinfitwxy(X,Y,zeros(size(X)),ones(size(Y))*1,mymodel,[1],0);
      
      clin(remST, bstrap,idx,:)=[BETA(1), EMI.BETACONF(:,1)', t];  
     
      if (t>=-6 & bstrap==1)
	figure(remST); if (t==-6) clf; hold on; end
	title(['removing N= ' num2str(remST-1) ' BS=' num2str(bstrap)])
        plot(X,Y,[col{mod(idx,6)+1} typ{mod(idx,10)+1}]);
        p=plot([1:150],mymodel(BETA,[1:150]),col{mod(idx,6)+1});
        set(p,'linewidth',2);
	if (t==0) 
	  prettify(gca); grid; ylim([0 1])
	  print('-dpng',['figs/monte-carlo-Correlation_bstrap1_removing_N' ...
			 num2str(remST-1) '.png'])
	end
      end
    ...
    % for each randomization, try to fit the time-scale
    X=squeeze((clin(remST,bstrap,:,4)-0.5)*step + T0);
    Xerr=zeros(size(X));
    Y=squeeze(clin(remST,bstrap,:,1));
    Yerr=squeeze((clin(remST,bstrap,:,3)-clin(remST,bstrap,:,2))*0.5);

    mymodel2=@(b,x) (b(1)*exp(x/b(2))+b(3));
    [BETA,R,J,COVB,MSE,EMI] = ...
	nlinfitwxy(X,Y,Xerr,Yerr,...
		   mymodel2,[-3e-3  4 -1e-3]);
    
    % save the time scale
    tau(remST,bstrap)=BETA(2);
    tauerr(remST,bstrap)=EMI.BETAERR(2);
    
    % if the first boot strap, make a sample plot
    if (bstrap==1)
      figure(100+remST); clf; hold on;
      p=errorbar(X, Y, Yerr, Yerr,'*k');

      xlabel('Time (h)')
      ylabel('Slope (1/km)')
      title(['N= ' num2str(remST-1) ' BS=' num2str(bstrap)]);
      prettify(gca); grid;
      
      finex=(min(X):0.1:max(X));
      p=plot(finex,mymodel2(BETA,finex),'r');
      set(p,'linewidth',2);
      ylim([-5e-3 0])
  ...
  prettify(gca); grid; 
  print('-dpng',['figs/monte-carlo-Slopes_N' num2str(remST-1) '.png'])
...
title([strrep(tag,'_',' ')]);
prettify(gca); grid; 
print('-dpng',['figs/monte-carlo-Slopes_all.png'])
...
plot([0:18], mtau,'k*-','linewidth',2)
plot([0:18], mtau+stau,'r-')
plot([0:18], mtau-stau,'r-')

plot([0:18], mtau+etau,'b-')
plot([0:18], mtau-etau,'b-')

xlabel('Number of stations removed (#)')
ylabel('Time-scale (h)')
prettify(gca); grid; 
print('-dpng',['figs/monte-carlo-scales_vs_removed.png'])
