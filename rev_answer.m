...
switch opt
 case 1
  ...
  ID='_14hr_interval_2011_2012'; step=1;
 case 2
  tag=['Daytime_Non_Convective_Events_2011_2012']; nevents=30; var='PWV';T0=0; ctt15clean=0; ymin=0.3;maxdist=150;
  ID='_12hr_interval_output'; step=1;
end
...
%title([strrep(tag,'_',' ')]);
prettify(gca); grid;
figname=['figs/rev_figure2_ALL_' num2str(step) 'h_correlation_distance'];
...
legend(['Convective, ' leg{1}],'',['Non-Convective, ' leg{2}],'','Location','SouthWest');

%myfit=['Nonlinear fit using model:' char(10) 'Slope(t) = A + B exp(t / $\tau$)' char(10) ...
%       'Daytime' char(10)...
%       'Convective, ' leg{1} char(10) 'Non-Convective, ' leg{2} ];
%
%h=annotation('textbox',[0.15 0.15 0.51 0.18],'string','', ...
%           'backgroundcolor',[1 1 1],'edgecolor',[0 0 0],'fontsize',12,...
%           'fitboxtotext','off','interpreter','latex');
%
%t=annotation('textbox',[0.15 0.15 0.55 0.21],'string', myfit, 'fontsize',12,...
%             'linestyle','none',...
...
figname=['figs/rev_figure3_ALL_1h_30min_angular_vs_hours'];
print([figname '.png'],'-dpng');
print([figname '.eps'],'-depsc2');
saveas(gca,[figname '.fig'],'fig');
